                    ...
                    <div class="box col-md-12">
                        <div class="box-inner">
                            <div class="box-header well" data-original-title="">
                                <h2><i class="glyphicon glyphicon-hdd"></i> 附件批量导出</h2>
                            </div>
...
--%>
                                         ...
                                         <form id="downloadForm" action="<c:url value="/file/downloadfile" />" method="get">
                                         <input type="hidden" name="filepath">
                                        </form>
                                        
                                        <form id="exportForm" action="<c:url value="/export/batchexport" />"
                                              method="post">
                                            ...
                                            <input type="hidden" name="treetype" value="all">
    ...
    $(document).ready(function () {
        ...
        $("a.exportSearch,a.exportSelected").on("click", function (e) {
           ...
           /* if (CheckIsExist(recordType, category)) {*/
            	if($("#exportForm input[name=selectids]:checked").length==0){
        			return ;
       			}
                ...
                $("#exportForm").find("[name='treetype']").val("notall");
                ...
                $("#exportForm").ajaxSubmit({
                    ...
                    success: function (data, s, xhr) {
                        if (data.paths.length != 0) {
                           $.each(data.paths,function(index,content){
                                 /*  $("#downloadForm input[name=filepath]").val(content);
                                  $("#downloadForm").submit(); */
                                  window.open(encodeURI(encodeURI('<c:url value="/file/downloadfile" />?filepath='+content)));
                           });
                            noty({
                                text: data.lostFile+"条无效附件！",
                                layout: "top",
                                type: "success",
                                timeout: 1000
                            });
        ...
        /* $("#jstree").delegate(".exportVolume a", "click", function (e) {
                ...
                $("#exportForm").find("[name='export']").val($(this).closest("tr").attr("id"));
                $("#exportForm").find("[name='treetype']").val("all");
                $("#exportForm").submit();

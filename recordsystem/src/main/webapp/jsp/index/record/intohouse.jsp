                            ...
                            <div class="box-content">
                                <div class="row">
                                    ...
                                    <div class="col-md-10">
                                        ...
                                        <form:form modelAttribute="recordSearchForm" cssClass="form-horizontal"   action="${searchActoin}">
...
<script type="text/html" id="intoHouseTemplate">
    <form id="intoHouseForm" name="intoHouseForm" method="POST"
          action="{{= url}}" class="form-horizontal"
          role="form"
          target="fileiframe" >
        <input name="id" type="hidden" value="{{= id}}"  >
        <input name="fonds" type="hidden" value="{{= fonds}}"  >
    ...
    <div class="table-content">
        <table class="table table-striped table-bordered responsive">
            ...
            <tr>
				...
				{{/each}}
                <td class="operation">
                    <a class="btn btn-success viewRecordDocument" title="查看具体信息" recordid="{{= id}}" type='_record'><i class="glyphicon glyphicon-check"></i></a>
				</td>
            </tr>
            {{/each}}
            {{/if}}
        ...
        <table class="table table-striped table-bordered responsive">
            ...
            <%--<td class="year" id="{{= year}}">{{= yearString}}</td>
                ...
                <td class="center">
					...
					<a href="#" class="btn btn-success pieceedit" title="编辑信息"><i class="glyphicon glyphicon-edit"></i></a>
                    <%--{{if tableid == null}}
                    ...
                    {{/if}}
                    <a class="btn btn-success intohouse" title="入库"><i class="glyphicon glyphicon-ok"></i></a>
                </td>
            </tr>
            {{/each}}
            {{/if}}
...
<script>
    ...
    $(document).ready(function () {
        ...
        var recordTypeName = "";
        $('#jstree').jstree();
        var showTables = function (isVolume) {
            var url = "<c:url value="/json/piece/manage" />";
            if (isVolume) {
            ...
            $.ajax({
                ...
                success: function (result) {
                    ...
                    $(".pieceedit").on("click", function (e) {
                        var recordid = $(this).closest("tr").attr("id");
                        var recordType = $("#recordSearchForm input[name=rtId]").val();
                        var fondsId = $("#recordSearchForm input[name=fondsId]").val();
                        var currentPattern = "1";
                        if(isVolume)
                        	currentPattern = "0";
                        window.location.href = "<c:url value='/record/reimport'  />" + "?recordId=" + recordid+"&pattern="+currentPattern + 
                        		"&recordTypeId=" + recordType + "&fondsId=" + fondsId ;
                    });
                    ...
                    $(".intohouse").on("click", function (e) {
                        ...
                        $.ajax({
                            ...
                            success:function(result){
                                ...
                                result.isVolume = isVolume;
                                result.id = recordid;
                                result.fonds =  fondsId;
                                result.recordtype = recordType;
                                $("#commonModal2 .modal-body").empty();
                                $("#commonModal2 .modal-header .head").text("归档入库");
                                result.url = isVolume == true ? '<c:url  value="/json/record/volumeedit"  />':'<c:url value="/json/record/pieceedit"  />';
                                $("#intoHouseTemplate").tmpl(result).appendTo($("#commonModal2 .modal-body"));
                                $("#commonModal2").modal("show");
                                $('#intoHouseForm').validate({
                    ...
                    function getDetail(recordid,type) {
                        ...
                        /* $.ajax({
                            type: "GET",
                            url: "<c:url value="/json/document/get"  />",
                            data: data,
                            success: function (result) {
                                console.log(result);
                                ...
                                $(".filepreview").on("click", function (e) {
                                    ...
                                    $.ajax({
                                        ...
                                        success: function (result) {
                                            console.log(result);
                                            window.document.getElementById('FILEIFRAME').src = '<c:url value="/"  />' + result.tempPath;
                                        }
                                    })
                                });
                            }
                   	 ...
                   	 $.get("<c:url value='/json/record/detail'  />", {recordid:recordid,recordtypeid:recordType,type:type}, function (data, s, xh) {
                   		 ...
                   		 if(documents!=null&&documents!=''){
                                for(var i=0;i<documents.length;i++){
                             	...
                             	if(getSuffix(documents[0].path)=='pdf'){
                                ...
                                for(var i=0;i<information.length;i++){
                                  	 if(information[i].fieldvalue!=null&&information[i].fieldvalue!=""&&information[i].fieldvalue!="null")
                                    	$("#checkModal .wordContent").append("<p>"+information[i].name+":<strong>"+information[i].fieldvalue+"</strong></p>");
                                     	...
                                     	$("#checkModal .wordContent").append("<p>"+information[i].name+":<strong>未填写</strong></p>");

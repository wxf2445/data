                                ...
                                <form id="recordForm" name="recordForm" method="POST"
                                    ...
                                    <fieldset>
                                        <div class="">
                                            ...
                                            <input id="file_upload" type="file" name="resFile"/>
                                            <label id="resFile-error"></label>
                                        </div>
                                        <h4>上传文件显示区域</h4>

                                        <div class="well col-md-12 filewell" id="filewell">
                                            <%--<div class="col-md-3">
                                                ...
                                                <div class="form-group">
                                                    <div class="col-sm-12">
                                                        <p><%--<a class="btn btn-info testCustomModal">测试弹出</a>--%><a
                                                                class="btn btn-info addInfo">添加信息</a><span
                                                                class="alert alert-warning addInfoAlert"
                                                                style="display: none;"><b>请先选择模板！</b></span>
                                                        </p>
                                                    </div>
                                                </div>
                                                    <%--<div class="form-group">
            ...
            <div class="modal-body">
				...
				<dl class="warnBox">
					...
					<dd>
						...
						<br>
						<span>图片类： ["png", "jpg", "gif", "bmp", "jpeg"]；</span>
						<br>
...
<script>
    ...
    $(document).ready(function () {
            ...
            var uploadify_onSelectError = function (file, errorCode, errorMsg) {
                ...
                switch (errorCode) {
                    ...
                    case SWFUpload.QUEUE_ERROR.INVALID_FILETYPE:
                        ...
                        break;
                    case SWFUpload.QUEUE_ERROR.QUEUE_LIMIT_EXCEEDED:
                        msgText += "文件个数超过限制，仅限 " + this.settings.queueSizeLimit;
                        break;
                    default:
        ...
        var onSelect = function (file) {
            console.log(file);
            $("#"+file.id+" .fileImg").attr("src",getImgSrc(file.name));
        }
        $('#file_upload').uploadify({
            'swf': '<c:url value="/resources/uploadify/uploadify.swf"  />',
            'uploader': '<c:url value="/document/previewupload"   />',
            'height': 50,
            'width': 250,
            'auto': false,
            'simUploadLimit':   2,
            //'formData': {'filename': $.trim($(".currentFile .filename").attr("title"))},
            'fileDataName': 'file',
            'buttonText': '选择上传文件...',
            'fileTypeExts': Uploadify_fileLimit.extension,
            'multi': true,
            'method': 'post',
            'queueSizeLimit': 5,
            'queueID':'filewell',
            'removeCompleted':false,
            'fileSizeLimit': Uploadify_fileLimit.size,
            ...
            'itemTemplate' :'<div class="col-md-3 onefile"  id="\${fileID}">\
            <input class="imgid" type="hidden" value="">\
                <div class="col-md-12">\
                <img src="" class="col-md-5 fileImg">\
                <span class="filename col-md-7" title="\${fileName}">\${fileName}</span>\
        </div>\
        <div class="col-md-12 center">\
                <input type="hidden" value="">\
                <a class="btn btn-primary preview">预览</a>\
                <a class="btn btn-danger delete" href="javascript:\$(\'#\${instanceID}\').uploadify(\'cancel\', \'\${fileID}\')">删除</a>\
                </div>\
                </div>',
                   /* '<div id="${fileID}" class="uploadify-queue-item col-md-3">\
                    <div class="cancel">\
                        <a href="javascript:$(\'#${instanceID}\').uploadify(\'cancel\', \'${fileID}\')">删除</a>\
                    </div>\
                    <span class="fileName">${fileName} (${fileSize})</span><span class="data"></span>\
                </div>',*/
            'onUploadSuccess': function (file, odata, response) {
                var data = JSON.parse(odata);
                console.log(data);
                console.log(file);
                console.log(response);
                var resourcePath =data.resourcePath;
                var localpath =data.localpath;
                var suffix =data.filesuffix;
                var prepath = getBaseName()  + resourcePath;
                console.info("resourcepath:" + resourcePath);
                console.log(localpath);
                console.log(prepath);
                $(document).find(".filepreviewarea").empty();
                var pre = "";
                if ($.inArray(suffix, PdfExtension) != -1 || $.inArray(suffix, OfficeExtension) != -1) {
                    ...
                    $(document).find(".filepreviewarea").append(pre);
                    ...
                    document.getElementById('tempiframe').src = "<c:url value='/jsp/pdf.jsp'/>?url=" + resourcePath;
                } else if ($.inArray(suffix, ImgExtension) != -1) {
                    pre = $("<div class='row center'> <img src='" + "<c:url value='/file/openfile.jsp'/>?filepath=" + resourcePath + "' style='width: 100%'> </div>");
                    $(document).find(".filepreviewarea").append(pre);
                } else if ($.inArray(suffix, VideoExtension) != -1) {
                    showVideo("/" + resourcePath);
                }else if($.inArray(suffix, MusicEXtension) != -1){
                    showMusic("/" + resourcePath);
                }
            }
        });

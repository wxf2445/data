                    ...
                    <div class="box col-md-12">
                        <div class="box-inner">
                            ...
                            <div class="box-content">
                                     ...
                                     <c:url value="/json/archives/notreturn" var="archivesSearchForm"></c:url>
                                                	...
                                                	<c:forEach var="archive" items="${page.items }" varStatus="">
		                                                <tr>
		                                                    ...
		                                                    <td id="th_operation">${archive.borrowPerson }</td>
                                                   			<th id="th_phone" >${archive.phone }</th>
		                                                    ...
		                                                    <td id="th_deadline">
		                                                    	<fmt:formatDate pattern="yyyy-MM-dd"
                                                                            value="${archive.deadline }"/>
                                                                            </td>
                                                            <td>
								        <button type="button" class="btn btn-primary" title="归还" onclick="returnArchives(${archive.id })"><i class="glyphicon glyphicon-import"></i>
								        </button>
				        									</td>
        ...
        <table class="table table-striped table-bordered responsive">
            ...
            {{else}}
            	{{each archives.page.items}}
            		<tr>
               			 <td class="borrowNo">{{= borrowNo}}</td>
               			 <td class="borrowunits">{{= borrowunits}}</td>
               			 <td class="borrowPerson">{{= borrowPerson}}</td>
               			 <td class="phone">{{= phone}}</td>
               			 <td class="title">{{= borrowTitle}}</td>
               			 <td class="borrowtime" >{{= borrowtime}}</td>
...
<script type="text/html" id="sureReturn">
    <div class="modal-content">
        <div class="modal-header">
            <button type="button" class="close" data-dismiss="modal">×</button>
            <h4 class="optitle">确认归还</h4>
        </div>
        <div class="modal-body">
            <div class="box-content">
                <form class="form-horizontal" method="post"
                           role="form">
                    <div class="form-group">
                        <label class="col-sm-4 control-label">归还人姓名</label>
                        <div class="col-sm-4">
                            <input id="returnPerson" class="form-control">
        ...
        <div class="modal-footer">
            <a href="javascript:void(0)" class="returnArchives btn btn-primary"><strong><i class="glyphicon glyphicon-share-alt"></i>
...
<script>
    ...
    function returnArchives(id){
        $('#sureReturn').tmpl().replaceAll($('#view .modal-content'));
        $('#view').modal("show");
        $(document).on('click', '.returnArchives', function (e) {
        	var returnPerson = $("#returnPerson").val();
       	    $.ajax({
	            type: "POST",
	            url: "<c:url value="/json/archives/returnArchives" />",
	            data: {id:id,returnPerson:returnPerson},
	            success: function (result) {
                	...
                	$(document).off('click','.returnArchives');
	            }
            });  

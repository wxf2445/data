                    ...
                    <div class="box col-md-12">
                        <div class="box-inner">
                            <div class="box-header well" data-original-title="">
                                <h2><i class="glyphicon glyphicon-hdd"></i> 密码修改</h2>
                            </div>
                            <div class="box-content">
								<div class="navbar-inner">
									<c:url value='/setPwd/save' var="setPwdAction" />
										<form:form modelAttribute="setPwdForm" cssClass="form-horizontal" role="form" action="${setPwdAction }">
										...
										<div class="form-group">
											<div class="col-sm-4">
												<button id="submitBtn" class="btn btn-primary pull-right" style="min-width:40%;">提交</button>
											</div>
...
form.validate({
	rules:{
		oldPwd:{
			required:true,
			remote:{
				url: '<c:url value="/setPwd/checkPwd"/>',     //后台处理程序 
				  type: "get",               //数据发送方式
		...
		validCode:{
			required:true,
			remote:{
				url: '<c:url value="/setPwd/checkValidCode"/>',     //后台处理程序 
				  type: "get",               //数据发送方式
				  dataType: "json",           //接受数据格式    
				  data: {                     //要传递的数据
					  validCode: function() {

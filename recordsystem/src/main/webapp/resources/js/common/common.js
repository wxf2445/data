...
VideoExtension = ["flv", "mp4","avi","mov"];
MusicEXtension = ["mp3", "wav"];
addPrefix = function (element) {
    return "*."+element;
}
AllExtension = OfficeExtension.map(addPrefix).concat(PdfExtension.map(addPrefix)).concat(ImgExtension.map(addPrefix)).concat(VideoExtension.map(addPrefix)).concat(MusicEXtension.map(addPrefix)).join(";"),
Uploadify_fileLimit = {
    extension: AllExtension,
    size:"100MB"
};
...
function logout() {
    $.ajax({
        type: "POST",
        url: getBaseName()+"json/user/logout",
        data: form.serialize(),
...
$(document).ready(function () {
    ...
    createMask = function (tip) {
        var result = {
            loadTip:tip
        }
        $("#loadTemplate").tmpl(result).appendTo("body");
        $(".mask").height($(document).height());
        $(".mask .maskTipArea").css("padding-top",$("body").height()/3);
    }
    removeMask = function () {
        $(".mask").remove();
    }
        ...
        if (files.length > 0) {
            for (var i = 0; i < files.length; i++) {
                ...
                if (file.size > fileLimit.filesize * 1000000) {
                    return false;
                } else if (fileLimit.extension.indexOf(file.name.substring(file.name.lastIndexOf(".") + 1).toLowerCase()) == -1) {
                    return false;
                }
            }
...
/*        if ($.inArray(suffix, ImgExtension) != -1) {
            ...
            var param = { imgid : imgid};
            var pre = $("#imgTemplate").tmpl(param);
            $(".filepreviewarea").append(pre);
            file = imgjson[imgid];
            $('.filepreviewarea img').attr('src', window.URL.createObjectURL(file));*/


            /*Caman(".filepreviewarea img", function() {
             //this.sunrise();
             this.contrast($("#range").val());
             this.render();
             });*/
            /*var clone = $(".filepreviewarea img").clone();*/
            /*var caimg = $(".filepreviewarea img")[0];
             var fiterimg = caimg.cloneNode();*/
            /*$("#contrast,#saturation,#sharpen,#brightness").on("change", function () {
                $(this).prev(".filtervalue").text($(this).val());
                //由于插件有bug，暂时使用这种方法。
               /!* $(".filepreviewarea .imgdiv").empty();
                $(".filepreviewarea .imgdiv").append(clone);*!/
                var context = $(this);
                Caman.DEBUG = ('console' in window);
                var contrast = Number($("#contrast").val());
                var saturation = Number($("#saturation").val());
                var sharpen = Number($("#sharpen").val());
                var brightness = Number($("#brightness").val());
                Caman(".filepreviewarea .showimg", function() {
                    //this.sunrise();
                    this.reset();
...
//        }
    ...
    $(document).delegate(".btn-file", "change", function (e) {
        ...
        for(var i=0;i<length;i++){
        	file_size += resfiles[i].files[0].size; 
        }
        if (file_size > fileLimit.filesize * 1000000) {
        	$("#resFile-error").removeClass("valid").addClass("error").html("文件总大小不得超过100M");
        	$(".btn-file input[type=file]:last").val("");
            return false;
        } 
        if (files) {
            if (checkFile(files)) {
    ...
    $(document).ajaxError(function (event, jqXHR, options, errorMsg) {
        ...
        //if(event.responseText.hasNoPermission != undefined&&event.responseText.hasNoPermission == true){
            noty({
                text: "暂无权限,可向管理员申请权限!",
                layout: "top",
                type: "failure",
...
/*$("iframe").contents().contextmenu(function() {
